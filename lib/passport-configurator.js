  ...
  switch (authType) {
    case 'mock':
      strategy = new AuthStrategy(_.defaults({}, options),
        ...
        // The user returned here is hard-coded in the mock strategy package.
        // Email will always be foo@bar.com.
        function(user, done) {
          ...
          self.userModel.findOne(query, function(err, user) {
            ...
            if (!user) {
                ...
                message: g.f(
                  'Could not find development user in the database. ' +
                  'Make sure you completed the seeding step.'
                ),
              });
              return;
            }
